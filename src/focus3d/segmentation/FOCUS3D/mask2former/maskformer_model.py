...
from .modeling.pixel_decoder.fpn3d import BasePixelDecoder3D
from .modeling.pixel_decoder.msdeformattn3d import MSDeformAttnPixelDecoder3D
from .modeling.transformer_decoder.mask2former_transformer_decoder_3d import (
...
class MaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # Build pixel decoder
        pixel_decoder_name = cfg.MODEL.SEM_SEG_HEAD.NAME

        if pixel_decoder_name == 'BasePixelDecoder3D':
            pixel_decoder = BasePixelDecoder3D(cfg, backbone.output_shape())
        elif pixel_decoder_name == 'MSDeformAttnPixelDecoder3D':
            pixel_decoder = MSDeformAttnPixelDecoder3D(
        ...
        # Encoder proposal loss, compatible with old configs.
        # If ENC_LOSS is not defined or False, these keys will not be added.
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            for b, (
                ...
                # 1) Query selection on LOW-RESOLUTION masks.
                #
                # instance_inference() now removes queries that are
                # mathematically impossible to survive downstream filtering.
                instance_r = self.instance_inference(
                    mask_cls_result,
                    mask_pred_result,
                )

                # 2) Interpolate ONLY surviving masks.
                ...
                if pred_masks.shape[0] > 0:
                    ...
                    # Avoid calling interpolate on K=0.
