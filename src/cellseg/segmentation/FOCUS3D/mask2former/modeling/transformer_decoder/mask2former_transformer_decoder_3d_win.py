...
def build_transformer_decoder(cfg, in_channels, mask_classification=True):
    ...
    if name != 'MultiScaleMaskedTransformerDecoder':
        raise ValueError(f'Unsupported transformer decoder: {name}')

    kwargs = MultiScaleMaskedTransformerDecoder.from_config(
        cfg,
        in_channels=in_channels,
        mask_classification=mask_classification,
    )
    return MultiScaleMaskedTransformerDecoder(**kwargs)
